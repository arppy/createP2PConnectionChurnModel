...
i = 1
predictions = {}
with open("trace_network100000xHourAll.csv") as f1, open("1010rr1101rr10network100000xHourAll10xprediction.out") as f2:
      ...
      for lineI in range(userLineI, len(trace[user]) - 1) :
        ...
        if int(trace[user][lineI][1]) == 1:
          ...
          if hourStamp < sessionStartUser :
            ...
            offline = False
            numberOfConnectedNeighbor = 0
            numberOfCennectionTryoutToOnlineNode = 0
            numberOfCennectionTryoutAtAll = 0
            dictOfAlreadyTriedNodes = {}
            while True :
              neighbor = str(random.randrange(NETWORK_SIZE))
              if neighbor not in dictOfAlreadyTriedNodes :
                dictOfAlreadyTriedNodes[neighbor] = 1
                  ...
                  if int(trace[neighbor][lineNeighborI][1]) == 1:
                    ...
                    if hourStamp < sessionStartNeighbor:
                      ...
                      try:
                        ...
                        numberOfCennectionTryoutToOnlineNode += 1
                        if predictions[featureVector] == "1":
                          numberOfConnectedNeighbor += 1
                        ...
                        print("ERROR", coreNumber, hourStamp, traceAssignment[user], lineI, sessionStartUser,
                              sessionEndUser,
                              str(trace[user][lineI]), str(trace[user][lineI + 1]),
                              traceAssignment[neighbor], lineNeighborI, sessionStartNeighbor, sessionEndNeighbor,
                              str(trace[neighbor][lineNeighborI]), str(trace[neighbor][lineNeighborI + 1]), "-->",
                              str(featureVector))
                      break
                    ...
                    continue
              numberOfCennectionTryoutAtAll+=1
              if numberOfCennectionTryoutAtAll > 2*NETWORK_SIZE or numberOfConnectedNeighbor >= EXPECTED_NUMBER_OF_CONNECTED_NEIGHBOR :
                break
            # this session could be good for next examination time
            print(core, hour, user, offline, connectedNeighborList, numberOfCennectionTryoutAtAll, numberOfConnectedNeighbor)
            break
          ...
          continue
      print(core, hour, user, offline, connectedNeighborList)
      if offline == False :
